...
The current app is a usable local social-feed prototype:

- capture a social post or web page manually
- start from X, Facebook, Reddit, or RSS-style presets
- record the platform, author, quoted claim, and annotation
...
This is not wired to social APIs or native share sheets yet. Manual social URL capture in the web app is the main path while the data model and routing remain flexible.
...
2. Add optional social importers for RSS first, then X/Facebook where API access allows it.
3. Add native share-sheet capture for URLs.
4. Add an in-app browser capture flow with Hypothesis-like selectors.
5. Add iPad/tablet split layouts for lens review and tag inspection.
6. Add encrypted sync hooks once the data-source contract is stable.
